...
export default function AdminView({ initialData }: { initialData: DashboardData }) {
  ...
  return <main className={styles.adminPage}>
    ...
    <section className={styles.records}>
      ...
      <div className={styles.recordList}>{pageRows.map((project) => <article key={project.id}><div><strong>{project.name}</strong><span>{project.status} · {project.subtitle}{project.date ? ` · ${project.date.slice(0, 10)}` : ''}</span></div><div><button onClick={() => openEdit(project)}>编辑</button><button className={styles.danger} onClick={() => remove(project)}>删除</button></div></article>)}</div>
